...
namespace XFood.API.CriticalFactors
{
    ...
    public class CriticalFactorController : Controller
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(string))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(CreateCriticalFactorResponse))]
        public async Task<ActionResult<Result<CreateCriticalFactorResponse>>> CreateCriticalFactor(
        [FromBody] CreateCriticalFactorRequest request,
        [FromServices] ICommandDispatcher commandDispatcher,
        CancellationToken cancellationToken
    )
        {
            var res = await commandDispatcher.Dispatch<CreateCriticalFactorRequest, Result<CreateCriticalFactorResponse>>(request, cancellationToken);
